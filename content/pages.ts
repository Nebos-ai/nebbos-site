  ...
  trust: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "Every enterprise SaaS contract contains language about data ownership, audit trails, and portability. In most cases, that language is written in a way that would be difficult to enforce if the vendor took a shortcut. Nebbos is architected so that shortcuts are structurally impossible, not contractually forbidden. Row-level client isolation at the substrate means an application-layer bug cannot leak your data to another client — the isolation is a database primitive, not a policy. Append-only attestation means an operator cannot rewrite history — the storage layer refuses updates. Portability is not a promise for offboarding day — the export routine runs on every deploy, so we know it works before you need it. Every trust claim becomes a property the code cannot violate, and the contract describes what the code already enforces.",
      },
      {
        id: "pillars",
        kind: "list-numbered",
        ...
        items: [
          ...
          { title: "Every request carries identity through row-level client isolation", body: "The substrate enforces client isolation at Layer 01 · Data, not at the application layer. Row-level security policies attached to every table check the request&rsquo;s identity claim against the row&rsquo;s client ownership. An application-layer bug that forgets to include a client filter cannot leak data — the database refuses the read. This is a structural property, not a code convention." },
          { title: "Every audit trail is portable to you if you leave", body: "The audit-trail export is a first-class capability, not a departure-day scramble. Any client requests a full audit-trail export at any time, in a machine-readable format (JSONL + independent verifier), that your inspector-general runs against the hash chain independently of Nebbos infrastructure. Because the export routine runs on every deploy, we know it works when you need it, not only after you have asked." },
        ],
      },
      {
        id: "ownership",
      ...
      },
      {
        ...
        items: [
          { title: "No shared-model training on your data", body: "Your preference pairs stay in your client. Your data does not train a model available to any other client." },
          { title: "No cross-client aggregation for our own benchmarks", body: "We do not compute aggregate metrics across clients for our own marketing or benchmarking." },
          { title: "No infrastructure-vendor change without notice", body: "The vendors Nebbos itself runs on (hosting, DNS, observability, payment) are disclosed at /legal/subprocessors. Enterprise customers get 30 days advance notice of any change. These vendors do not touch customer data — that lives on the customer's Cradle." },
      ...
      },
      {
        ...
        items: [
          { title: "How does row-level isolation actually work?", body: "Postgres row-level security policies attached to every table. Every request carries a client identity in a request-scoped context variable. RLS policies compare that identity against the row&rsquo;s client column. An application-layer bug that forgets the filter fails closed — the database refuses the read." },
