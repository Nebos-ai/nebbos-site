    ...
    <div style={{ "--tint": tint, "--spot": tint } as CSSProperties}>
      ...
      <Section labelledBy={`others-${k}-h`}>
        ...
        <Stagger className="mt-14 grid gap-4 md:grid-cols-3 lg:gap-5" step={0.1}>
          {others.map((p) => {
            const t = PRODUCT_TINT[p.key];
            return (
              <StaggerItem key={p.key}>
                <Link
                  href={`/products/${p.slug}`}
                  style={{ "--tint": t, "--spot": t } as CSSProperties}
                ...
                >
                  ...
                  <div className="relative mt-auto pt-12">
                    <p className="m-0 font-code text-[11px] font-medium uppercase tracking-label text-ink-3">{p.eyebrow}</p>
                    <h3 className="m-0 mt-2 font-display text-[clamp(1.6rem,2.2vw,2rem)] font-medium tracking-tight text-ink">
                      {shortName(p)}
                    </h3>
                    <p className="m-0 mt-2 text-[15px] leading-relaxed text-ink-2">{p.tagline}</p>
                  </div>
                  <span className="relative mt-6 inline-flex items-center gap-2 self-start rounded-pill bg-white/[0.05] py-1.5 pl-4 pr-1.5 font-code text-[11px] font-medium uppercase tracking-label text-ink ring-1 ring-rule ring-inset transition-colors group-hover:bg-white/[0.09]">
                    Open
                    <span
                      className="grid size-6 place-items-center rounded-pill bg-[var(--tint)] text-[12px] text-white"
                      aria-hidden
                    >
                      →
                    </span>
